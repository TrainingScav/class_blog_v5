...
public class BoardController {
    ...
    public String updateForm(@PathVariable(name = "id") Long boardId,
        ...
        if (board == null) {
            throw new Exception404("수정할 게시글이 존재하지 않습니다.");
        }
    ...
    @GetMapping("/board/{id}")
    public String detail(@PathVariable(name = "id") Long boardId, HttpServletRequest request) {

        log.info("게시글 상세 보기 요청 - ID : {}", boardId);

        Board board = boardRepository.findById(boardId);
        log.info("계시글 상세보기 조회 완료 - 제목 : {}, 작성자 : {} ", board.getTitle(), board.getUser().getUsername());
